...
better). The \emph{Behav.}\ column indicates the behavioral power
model: ``--'' = constant power, ``param'' = parametric climb effort
and descent decay, ``emp'' = empirical $\pi(g)$.}
\label{tab:buildup}
...
\quad + relevance filter
  & {\cmark} & {param} & {split} & {\cmark} & {--}
  & 6.7 & 15.1 & 11.6 \\
\quad + ride power curve
  & {\cmark} & {emp} & {split} & {\cmark} & {ride}
  & 6.7 & 14.5 & 10.6 \\
\quad + cached profile
  & {\cmark} & {emp} & {split} & {\cmark} & {cached}
  & 6.7 & 14.7 & 9.9 \\
\bottomrule
\end{tabular}
\end{table}
...
moving average everywhere (31.4 vs 29.9 on mountains, 8.7 vs 7.1 on
flat): its descent predictions are so wrong that the extra gradient
...
(11.6). This is the core contribution: by learning only from segments
whose predicted traversal time exceeds 120\,\si{\second}, the integral
corrections avoid contamination from short punchy efforts that do not
represent the rider's sustained-climb behavior.
