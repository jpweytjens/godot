...
Oracle naive ($v_\text{true}$, no physics)
  & 1.8 & 2.7 & 3.8 & 5.6 & 11.3 \\
Oracle corrections (physics, perfect $\Delta$)
  & 2.8 & 3.5 & 5.0 & 5.3 & 8.2 \\
\midrule
Duration-bucketed (real estimator)
  & 5.8 & 8.7 & 10.5 & 14.3 & 12.1 \\
Naive with prior ($v = \SI{29}{\km\per\hour}$)
  & 6.7 & 9.4 & 10.6 & 17.0 & 22.2 \\
\bottomrule
\end{tabular}
\end{table}
...
naive achieves only 5.6\,\si{\minute} on hills and 11.3 on
...
No constant-speed model can go below this floor. It is lowest on
pancake rides (1.8\,\si{\minute}, steady riding) and highest on
mountains (11.3, where the speed swings between 15\,\si{\km\per\hour}
climbing and 40\,\si{\km\per\hour} descending are sustained and
large).

\paragraph{Physics helps --- when calibrated.}
Oracle corrections beat oracle naive on every category: 5.3 vs 5.6 on
hills, 8.2 vs 11.3 on mountains. The physics model \emph{does}
reduce error below the constant-speed floor, because it predicts
per-segment speed rather than projecting one average. On mountains
the gain is 3.1\,\si{\minute} --- gradient awareness earns its keep
by anticipating the speed difference between sustained climbs and
descents. On hills the gain is small (0.3\,\si{\minute}) but
positive: even on mixed terrain, per-gradient predictions outperform
the aggregate when the corrections are perfectly calibrated.

\paragraph{Convergence is the dominant cost.}
The gap between oracle corrections and the real estimator is
9.0\,\si{\minute} on hills and 3.9 on mountains. This is pure
convergence overhead: the integral corrections have not reached their
final values while the ride is in progress. On hills, where rides
are short and climb durations diverse, the corrections learn slowly;
on mountains, where one long climb dominates, they converge quickly.
The naive-with-prior baseline (17.0 on hills) confirms that
\emph{not} using physics is worse than using imperfectly calibrated
physics --- the gradient awareness saves 2.7\,\si{\minute} even
during the convergence period.

The practical implication is that the largest remaining gains lie not
in better physics or more correction channels, but in
\emph{faster convergence of the corrections toward their final
values}. Cross-ride parameter learning
(Section~\ref{sec:cross-ride}), warm-start priors, and accurate
$v_\text{flat}$ estimation are the levers that close this gap.
Every minute shaved from the convergence period is a minute of
improved prediction on every ride, across every difficulty tier.
